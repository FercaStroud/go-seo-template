...
.container.mx-auto.flex.flex-wrap.justify-center.mb-5
  ...
  .flex.flex-col.justify-center.items-center.text-center.mb-5
      ...
      template(v-for="(post, key) in $store.state.posts", :key="key")
        template(v-if="key < 3")
          .basis-full.p-2(class="basis-1/3")
            .bg-sober-alt.p-5
              .img-container.flex.justify-center.items-center.mb-2
                img.w-full.h-auto(
              ...
              .flex.justify-center.items-center.h-20.mb-2
                h3.title-primary-sober.font-black.text-md(class="sm:text-xl") {{ post.title.length >= 45 ? post.title.substring(0, 42) + '...' : post.title }}
              .h-48.overflow-hidden.mb-5
                .ql-editor(v-html="post.description")
              .flex.justify-center.items-center
                a.primary-button-sober(href="/blog") ir a blog
</template>
